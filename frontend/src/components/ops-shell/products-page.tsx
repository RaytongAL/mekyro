...
import { CategoryCascader, buildCategoryPath, type CategoryOption } from "./category-tree";
import { CategoryManageDrawer } from "./category-manage-drawer";
...
export function ProductsPage() {
  ...
  const fetchProducts = useCallback(() => {
    ...
    fetch(`/api/internal/products/?${params.toString()}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  }, [page, pageSize, search, statusFilter, stockFilter, selectedWorkspaceId, categoryIdParam, ordering, t]);

  const fetchCategories = useCallback(() => {
    if (!selectedWorkspaceId) return;
    const token = getToken(); if (!token) return;
    const params = new URLSearchParams({ workspace_id: selectedWorkspaceId });
    fetch(`/api/internal/categories/?${params.toString()}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data?.code === 200 && data.data) {
          const nextCategories = (Array.isArray(data.data)
            ? data.data
            : data.data.results ?? []) as CategoryOption[];
  ...
  useEffect(() => {
    ...
    setProductSkus({});
  }, [selectedWorkspaceId, search, statusFilter, stockFilter, categoryIdParam, ordering, page, pageSize]);
  ...
  function fetchProductSkus(productId: number) {
    ...
    fetch(`/api/internal/products/${productId}/`, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data?.code === 200 && data.data) {
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/internal/products/create/", {
        method: "POST",
  ...
  function openProductEdit(product: ProductItem) {
    ...
    const token = getToken();
    if (!token) return;
    fetch(`/api/internal/products/${product.id}/`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
  ...
  async function handleEdit() {
    ...
    try {
      const res = await fetch(`/api/internal/skus/${editTarget.id}/update/`, {
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/internal/products/${deleteTarget.product_id}/delete/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (data?.code === 200) {
  ...
  async function handleProductEdit() {
    ...
    try {
      ...
      const res = await fetch(`/api/internal/products/${productEditTarget.id}/update/`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
  ...
  async function handleSpecSave() {
    ...
    try {
      const res = await fetch(`/api/internal/products/${specTarget.id}/update/`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
  ...
  async function handleAddSku() {
    ...
    try {
      const res = await fetch("/api/internal/skus/create/", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleInventoryOp() {
    ...
    try {
      const res = await fetch(`/api/internal/skus/${invTarget.skuId}/inventory-logs/create/`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({
          type: invType,
  ...
  async function confirmSkuDelete() {
    ...
    setSkuDeleting(true);
    try {
      const res = await fetch(`/api/internal/skus/${skuDeleteTarget.skuId}/delete/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (data?.code === 200) {
  ...
  function fetchTrash() {
    const token = getToken(); if (!token || !selectedWorkspaceId) { setTrashLoading(false); return; }
    setTrashLoading(true);
    const params = new URLSearchParams({ page: String(trashPage), page_size: String(trashPageSize) });
    params.set("workspace_id", selectedWorkspaceId);
    const url = trashTab === "products" ? `/api/internal/products/trash/?${params.toString()}` : `/api/internal/skus/trash/?${params.toString()}`;
  ...
  useEffect(() => { if (showTrash) fetchTrash(); }, [showTrash, trashPage, trashPageSize, selectedWorkspaceId, trashTab]);

  function doTrashRestore(id: number) {
    const token = getToken(); if (!token) return;
    setTrashActing(true);
    const url = trashTab === "products" ? `/api/internal/products/${id}/restore/` : `/api/internal/skus/${id}/restore/`;
  ...
  function doTrashPermanentDelete(id: number) {
    const token = getToken(); if (!token) return;
    setTrashActing(true);
    const url = trashTab === "products" ? `/api/internal/products/${id}/delete/?permanent=true` : `/api/internal/skus/${id}/delete/?permanent=true`;
  ...
  async function handleCreateSub(parentId: number, name: string) {
    const token = getToken(); if (!token || !selectedWorkspaceId) return;
    const res = await fetch("/api/internal/categories/create/", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ name, parent_id: parentId || null, workspace_id: selectedWorkspaceId, sort_order: 0 }),
    });
    const data = await res.json();
  ...
  async function handleDeleteCategory(categoryId: number) {
    const token = getToken(); if (!token) return;
    const res = await fetch(`/api/internal/categories/${categoryId}/delete/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json();
    if (data?.code !== 200) throw new Error(data?.message);
  ...
  async function handleMoveCategory(categoryId: number, newParentId: number | null) {
    const token = getToken(); if (!token) return;
    const res = await fetch(`/api/internal/categories/${categoryId}/update/`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
  ...
  return (
    <div className={styles.productsLayout}>
                        ...
                        <TableCell colSpan={PRODUCT_COL_COUNT} className={styles.productSkuExpansionCell}>
                          <div className={styles.productSkuPanelHeader}>
                            ...
                            <Button type="button" variant="outline" size="sm"
                              disabled={!(product.spec_template && product.spec_template.length > 0)}
                              title={!(product.spec_template && product.spec_template.length > 0) ? t("ops.productsNeedSpecTemplate") : undefined}
            ...
            <div className={styles.formGroup}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {editSkuImage.map((img, i) => (
                  <div key={i} style={{ position: "relative", width: 80, height: 80, borderRadius: 8, overflow: "hidden", border: "1px solid var(--border-subtle)" }}>
                    ...
                    <button type="button" onClick={async () => {
                      ...
                      const token = getToken(); if (!token) return;
                      await fetch(`/api/internal/products/${editTarget.product_id}/images/${img.id}/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }).catch(() => {});
                      setEditSkuImage([]);
                ...
                {editSkuImage.length < 1 ? (
                  <label style={{ width: 80, height: 80, border: "2px dashed var(--border-default)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: uploading ? "wait" : "pointer", opacity: uploading ? 0.5 : 1 }}>
                    <span style={{ fontSize: 24, color: "var(--text-faint)" }}>{uploading ? "⏳" : "+"}</span>
                    <input type="file" accept="image/*" style={{ display: "none" }} disabled={uploading} onChange={async (e) => {
                      const f = e.target.files?.[0]; if (!f || !editTarget) return; e.target.value = "";
                      setUploading(true);
                      try {
                        const url = await uploadImage(f);
                        const token = getToken(); if (!token) return;
                        const res = await fetch(`/api/internal/products/${editTarget.product_id}/images/`, {
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {productEditImages.filter((i) => i.type === "product").map((img, i) => (
                  <div key={i} style={{ position: "relative", width: 80, height: 80, borderRadius: 8, overflow: "hidden", border: "1px solid var(--border-subtle)" }}>
                    ...
                    <button type="button" onClick={async () => {
                      ...
                      if (img.id) {
                        const token = getToken(); if (!token) return;
                        await fetch(`/api/internal/products/${productEditTarget.id}/images/${img.id}/`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }).catch(() => {});
                      }
                ...
                {productEditImages.filter((i) => i.type === "product").length < 5 ? (
                  <label style={{ width: 80, height: 80, border: "2px dashed var(--border-default)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: uploading ? "wait" : "pointer", opacity: uploading ? 0.5 : 1 }}>
                    <span style={{ fontSize: 24, color: "var(--text-faint)" }}>{uploading ? "⏳" : "+"}</span>
                    <input type="file" accept="image/*" style={{ display: "none" }} disabled={uploading} onChange={async (e) => {
                      const f = e.target.files?.[0]; if (!f || !productEditTarget) return; e.target.value = "";
                      setUploading(true);
                      try {
                        const url = await uploadImage(f);
                        const token = getToken(); if (!token) return;
                        const res = await fetch(`/api/internal/products/${productEditTarget.id}/images/`, {
                          method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      ...
      <CategoryManageDrawer open={manageDrawerOpen} onOpenChange={setManageDrawerOpen} onChanged={fetchCategories} />
      <InventoryLogsDrawer
        open={!!logsSku}
  ...
  return (
    <>
      <Button
        ...
        size="icon"
        disabled={uploading}
        aria-label="选择图片 / Choose image"
        title="选择图片 / Choose image"
...
function ImageUploader({
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
      ...
      {images.length < max ? (
        <ImageFileButton uploading={uploading} onSelect={onUpload} />
      ) : null}
    </div>
  );
}
